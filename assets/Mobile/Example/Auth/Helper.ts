...
export async function getSolanaTransaction(from: string, receiver: string, amount: number) {
    // mock a solana native transaction
    // send some native on solana devnet

    const obj = { sender: from, receiver: receiver, lamports: amount };
...
export async function getSplTokenTransaction(from: string, receiver: string, amount: number, mintAddress: string) {
    // mock a solana spl token transaction
    // send some spl token on solana devnet

    const obj = { sender: from, receiver: receiver, amount: amount, mint: mintAddress };
    const rpcUrl = 'https://rpc.particle.network/';
    const pathname = 'solana';
    const chainId = 103;
